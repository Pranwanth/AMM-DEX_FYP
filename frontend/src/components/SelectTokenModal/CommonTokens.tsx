...
const CommonTokens = (props: Props) => {
    ...
    return <div className="flex gap-1  flex-wrap">
        {commonTokens.map((token, index) => {
            return <div key={`token-${index}`} className="py-1 px-2 rounded-2xl border-2 border-sky-800/20 font-medium" onClick={() => { setTokens(tokenType, token); setIsModalOpen(false) }}>{token}</div>
        })}
    </div>
}
